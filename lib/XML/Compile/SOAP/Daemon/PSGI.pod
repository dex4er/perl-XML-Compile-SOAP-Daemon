...
This module handles the exchange of SOAP messages via L<PSGI> stack, using
L<Plack> toolkit.
...
When defined, the CODE will get called with a L<Plack::Request> object before
processing SOAP message.

=item postprocess => CODE

When defined, the CODE will get called with a L<Plack::Request> and
L<Plack::Response> objects after processing SOAP message.
...
The same as B<to_app> but accepts additional B<preprocess> and B<postprocess> options.
...
The code and documentation for this module was contributed by Piotr Roszatycki
in March 2012.
